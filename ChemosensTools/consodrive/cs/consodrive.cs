...
namespace ChemosensTools.ConsoDrive
{
    public class ConsoDrive
    {
        ...
        public static string LoginURL(string dataDirPath, string id, string hashKey)
        {
            string json = "";
        ...
        public static string NewLogin(string dataDirPath, string hashKey, string json, string infos, string mail)
        {
            //TimeSens.webservices.helpers.RandomStringGenerator random = new RandomStringGenerator();
            //random.IncludeChars = true;
            //random.IncludeNumerics = true;
            //random.NbCarMax = 6;
            //random.NbCarMin = 6;
            ...
            //    file = dataDirPath + code + ".json";
            //}
            File.Create(file).Dispose();
            File.WriteAllText(file, Serializer.Encrypt(System.Uri.UnescapeDataString(json), hashKey));

            string url = ConfigurationManager.AppSettings["ConsoDriveURL"];

            // Envoi mail avec code au juge
            string txt = "<p>Bonjour, et merci pour votre inscription !</p>";
            txt += "<p>Installez-vous de préférence sur un ordinateur (l’affichage du questionnaire y sera optimisé). Assurez-vous d’avoir accès depuis cet ordinateur aux fichiers PDF des factures Drive que vous souhaitez nous envoyer. Il vous suffit alors de cliquer sur le lien ci-dessous. Si le lien ne fonctionne pas, copiez l’adresse suivante et collez-la dans votre barre de navigation.</p>";
            txt += "<p><a href='" + url + "?id=" + code + "'>Lien vers l'étude consodrive</a>.</p>";
            txt += "<p>La participation à cette étude sera indemnisée sous forme d’un bon dématérialisé d’une valeur de 10€ qui vous sera envoyé par mail à la fin de l’étude, après confirmation de votre adresse email personnelle. Attention, le versement de l'indemnisation n'est pas immédiat.</p>";
            txt += "<p>Attention, ce lien est personnel. Ne le transmettez pas à une autre personne. Si quelqu’un de votre entourage souhaite participer à cette étude, il/elle doit au préalable s’inscrire ici : https://www.chemosenstools.com/consodrive/inscription.html.</p>";
            txt += "<p>Pour tout problème ou toute question, n’hésitez pas à nous contacter à l’adresse consodrive@inrae.fr : nous nous tenons à votre disposition pour vous aider !</p>";
            txt += "<p>Nous vous remercions par avance pour votre participation !</p>";
            txt += "<p></p><p>Consodrive – Plateforme Chemosens</p><p>Centre des Sciences du Goût et de l’Alimentation</p>";

            MailHelper.SendMail2(mail, "Votre lien d'accès pour l'étude consodrive", txt, "consodrive@inrae.fr", "consodrive@inrae.fr");

            // Envoi mail à consodrive
            infos = System.Uri.UnescapeDataString(infos);
            infos += "<p>Code : " + code + "</p>";
            MailHelper.SendMail2("consodrive@inrae.fr", "Nouvelle inscription", infos , "consodrive@inrae.fr", "consodrive@inrae.fr");
                                    ...
                                    foreach (JToken t in jObject["Invoices"].Children())
                                    {
                                        ...
                                        foreach (JToken x in t["ListItems"].Children())
                                        {
                                            //if (x["PUHT"] != null && x["PUHT"].ToString() != "0")
                                            {
                                                ...
                                                {
                                                    factureAlimentSheet.Cells[indexFactureAliment, 3].Value = x["Designation"].ToString().Replace("\r\n", "").Replace("\r", "").Replace("\n", "");
                                                }
                                ...
                                if (jObject["AccesQuestionnaire"] != null)
                                {
                                    foreach (JToken t in jObject["AccesQuestionnaire"].Children())
                                    {
                                        ...
                                        if (t["Date"] != null)
                                        {
                                            ...
                                            connexionSheet.Cells[indexConnexion, 3].Value = oldDate.AddHours(2).ToString("yyyy/MM/dd");
                                            if (indexConnexion > 2)
                                            {
                                ...
                                if (jObject["Dimensions"] != null)
                                {
                                    foreach (JToken x in jObject["Dimensions"].Children())
                                    {
                                        ...
                                        foreach (JToken y in x["SousDimensions"].Children())
                                        {
                                            ...
                                            {
                                                reponses.Add(z.ToString());
                                                //sousDimensionSheet.Cells[indexSousDimension, 7].Value += z.ToString() + ";";
                                            }
                                            sousDimensionSheet.Cells[indexSousDimension, 7].Value = String.Join(";", reponses.Distinct());
                                ...
                                if (jObject["QuestionsAliment"] != null)
                                {
                                    foreach (JToken x in jObject["QuestionsAliment"].Children())
                                    {
                                        if (x["Consomme"] != null && Convert.ToInt32(x["Consomme"].ToString()) > -1)
                                        {
                                            ...
                                            if (x["Designation"] != null)
                                            {
                                                alimentSheet.Cells[indexAliment, 2].Value = x["Designation"].ToString().Replace("\r\n","").Replace("\r", "").Replace("\n", "").Trim();
                                            }
                                            if (x["Consomme"] != null)
                                            {
                                            ...
                                            foreach (JToken y in x["Dimensions"].Children())
                                            {
                                                if (x["Choisi"].ToString() != "non")
                                                {
                                                    ...
                                                    if (x["Designation"] != null)
                                                    {
                                                        dimensionAlimentSheet.Cells[indexDimensionAliment, 2].Value = x["Designation"].ToString().Replace("\r\n", "").Replace("\r", "").Replace("\n", "").Trim();
                                                    }
                                                    if (y["Nom"] != null)
                                                    {
