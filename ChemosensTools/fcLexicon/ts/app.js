...
var FcLexiconApp = /** @class */ (function (_super) {
    ...
    FcLexiconApp.prototype.start = function (fullScreen) {
        ...
        if (this.requirements.AppUrl == 'http://localhost:44301/fcLexicon') {
            this.login = "mvisalli";
            this.password = "mvisalli";
            this.showMain();
        }
        else {
            this.showModalLogin();
    ...
    FcLexiconApp.prototype.showMain = function () {
        ...
        self.CallWCF('TesteLexique', { name: self.login }, function () {
            ...
            self.show("html/Main.html", function () {
                ...
                self.checkTestDescripteur.Check();
    ...
    FcLexiconApp.prototype.cleanDesignation = function (description) {
        ...
        self.CallWCF('GetSensoryWords', { txt: description, options: JSON.stringify(self.analysisOptions) }, function () {
            ...
            result.Mots.forEach(function (m) {
                MotsRetenus += m.Word + " ";
            });
            var table = new Framework.Form.Table();
            table.CanSelect = false;
            table.ShowFooter = false;
            table.ListData = result.Mots;
            table.Height = "40vh";
            table.FullWidth = true;
            table.AddCol("Word", "Mot(s)", 100, "left");
            //table.AddCol("AttributeFR", "Descripteur", 100, "left");
            //table.AddCol("FullConceptFR", "Concept complet", 300, "left");
            table.AddCol("Type", "Type", 200, "left");
            //table.AddCol("ConceptFR", "Concept", 100, "left");
            //table.AddCol("Modality", "Modalité", 100, "left");
            //table.AddCol("Preposition", "Quantifieur", 100, "left");
            ...
            //self.spanMotsNonRetenus.SetHtml(result.MotsNonRetenus.join(" "));
            //self.spanMotsRetenus.SetHtml(MotsRetenus);
            //self.spanDesignationNettoyee.SetHtml(result.TexteNettoye);
        });
    };
    return FcLexiconApp;
}(Framework.App));
