        ...
        this.show("html/DivDate.html", function () {
            ...
            var btnUpload = Framework.Form.Button.Register("btnUpload", function () { return true; }, function () {
                Framework.FileHelper.BrowseBinaries("", function (binaries, filename) {
                    var timeStamp = (new Date()).toISOString().replace(/[^0-9]/g, '').slice(0, -3);
                    var fn = self.questionnaire.Code + "_" + timeStamp;
                    self.CallWCF('TeleverseImageCodAppro', { base64string: binaries, filename: fn }, function () { }, function (res) {
                        self.aliment.Image = fn;
                        if (res.ErrorMessage != "") {
            ...
            var inputLieu = Framework.Form.InputText.Register("inputLieu", self.lieuTicket, Framework.Form.Validator.NotEmpty(), function (lieu) {
                ...
                for (var i = 0; i < lieux2.length; i++) {
                    if (lieux2[i].Key === lieu) {
                        ...
                        var typeCommerce = lieux2.filter(function (x) { return x.Key == lieu; })[0].Value;
